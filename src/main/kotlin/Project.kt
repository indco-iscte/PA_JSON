...
class JSONObject(val clazz: Any){ //varargs
    ...
    fun remove(pair: Pair<String, Any>, nome:String?) {
        var ind = 0
        var lista = false
        println("fora pair.first ${pair.first} e pair.second ${pair.second} e nome $nome")
        if(nome == null || nome.split(" ").size <2){
            valores.forEach{
                println("it.first ${it.first} e pair.first ${pair.first}")
                if(it.first.equals(pair.first) ){
                    println("do you even go here")
                    var holds : Any = ""
                    when(it.second) {
                    ...
                    println("holds "+ holds)
                    println("it.second ${it.second} e pair.second ${pair.second}")
                    if(!lista){
            ...
            println("valores antes "+valores.toString())
                    ...
                    ((valores[counter].second as JSONCollection).collect as MutableList<*>).forEach {
                        ...
                        if (it !is JSONCollection){
                            ...
                            split.forEach {
                                ...
                                if(it.equals(juntos)) {
                                    println("juntos: $juntos")
                                    b_dentro = true
                                }
                            }
                            println("split: $split size: ${split.size} e dentro: $dentro e b_dentro")
    ...
    fun updateJSONObject(pair: Pair<String, Any>,lista: String?){
        ...
        var ind = 0
        var old : Any? = ""
        println("estou dentro do update")
        println("$pair + $lista")
        valores.forEach{
            if(lista == null){
                if(it.first.equals(pair.first)) {
            ...
            if(lista != null){
                ...
                if(old_array::class.toString().contains("Aluno"))
                    ...
                    (a as Collection<*>).forEach {
                        println("this shit is $it")
                        if((it as Pair<*,*>).first!!.equals(pair.first)) {
                            ind = a.indexOf(it)
                            println("o index no ahaha é $ind e it é $it")
                        }
                    }
                    aux_contrs.clear()
    ...
    fun updateValoresList(pair: Pair<String, Any>, index: Int, nome: String?, undo: Boolean = false, obj: Any? = null){
        ...
        if (!undo) {
            ...
            var ind = 0
            var added = false
            //print("INDEX" + nome!!.split(" ")[1].toInt())
            for (i in valores){
        ...
        this.valores.forEach {
            ...
            when (it.second) {
                ...
                // come back to this dps de apagar os outros elems
                    ...
                    println("\n it.first: ${it.first} here:"+it.second+".")
